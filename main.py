...
async def clear(ctx, amount=5):
    await ctx.channel.purge(limit=amount
                            )  #Will clear messages from the Discord Channel
...
async def kick(ctx, member: discord.Member, *, reason="No Reason Provided"):
    await member.send("You have been kicked from the Community, Because:" +
                      reason)
...
async def ban(ctx, member: discord.Member, *, reason="No reason Provided"):
    ...
    await member.send(member.name +
                      "has been banned from the Community, because:" + reason)
...
async def unban(ctx, *, member):
    ...
    for ban_entry in banned_users:
        ...
        if (user.name, user.discriminator) == (member_name,
            ...
            await member.send(member.name +
                              "has been unbanned from the Community!")
            return
...
async def poll(ctx, *, content: str):
    ...
    embed = discord.Embed(
        title=f"{content}",
        description="React to this message with ✅ for yes, ❌ for no.",
        color=0xd10a07)
...
async def invite(ctx, *argument):
    invitelink = (
        "https://discord.com/oauth2/authorize?client_id=999521375863713892&permissions=8&scope=bot"
    )
